...
export const totalTransactions = () => DB.Transaction.count();

export const totalVotes = () => DB.Vote.count();

export const totalProposals = () => DB.Proposal.count();

export const totalBlocks = () => DB.Block.count();

export const latestEpoch = () => DB.Epoch.max("id");

export const latestEpochFromBlock = () => DB.Block.max("epoch");
...
export const overview = ({ limit = 10 } = {}) =>
  intoRecord({
    totalBlocks,
    oldestBlock,
    latestBlock,
...
export const epochs = ({ limit = 10, before, after }) =>
  before
    ? epochsBefore({ before, limit })
    : after
    ? epochsAfter({ after, limit })
    : epochsLatest({ limit });

export const epochsLatest = ({ limit = 10 }) =>
  DB.Block.findAll({
...
export const epochsBefore = ({ limit = 10, before }) =>
  DB.Block.findAll({
...
export const epochsAfter = ({ limit = 10, after }) =>
  DB.Block.findAll({
...
export const becomeValidatorCount = async ({ address = "" }) =>
  await toCount(
    db.query(
      `
...
export const changeValidatorMetadataCount = async ({ validator = "" }) =>
  await toCount(
    db.query(
      `
  SELECT COUNT(*) FROM "transactions"
...
export const deactivateValidatorCount = async ({ address = "" }) =>
  await toCount(
    db.query(
      `
  SELECT COUNT(*) FROM "transactions"
...
export const bondCount = async ({ source = "", validator = "" }) =>
  await toCount(
    db.query(
      `
  SELECT COUNT(*) FROM "transactions"
...
export const unbondCount = async ({ source = "", validator = "" }) =>
  await toCount(
    db.query(
      `
  SELECT COUNT(*) FROM "transactions"
...
export const txWithAddressCount = async ({ address = "" }) =>
  await toCount(
    db.query(
      `
        SELECT COUNT(*) FROM "transactions"
...
export const validatorNamadaAddressToConsensusAddresses = async (
  namadaAddress
) => {
  const addresses = new Set();
  if (namadaAddress) {
...
export const defaultAttributes = (args = {}) => {
  ...
  if (args instanceof Array) {
    ...
    if (args.include)
      attrs.include = [...new Set([...(attrs.include || []), ...args.include])];
    if (args.exclude)
      attrs.exclude = [...new Set([...(attrs.exclude || []), ...args.exclude])];
  } else {
